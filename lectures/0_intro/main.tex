...
\begin{itemize}
  \item roughly \daiji{one assignment every three weeks} in Rust
  \item a \daiji{midterm exam}
  \item a \daiji{final project} you will choose,
...
\begin{frame}{Programming languages}
  ...
  Depending on what the \daiji{task} is some tools are better\\
  \emph{Programming Language Concepts} help us choose the best tool for
  a use-case
  \bigskip
  \pause

  Rust is \daiji{not} the ultimate tool for all tasks\\
  but it's a \daiji{very rich} language with \daiji{recent},
  \daiji{interesting} concepts
  \smallskip
  \begin{itemize}
    \item \daiji{relevant} for desktop / server software
    \item \daiji{irrelevant} for client-side web dev
  \end{itemize}
\end{frame}
...
\begin{frame}{Problem}
  ...
  Most of them
  \begin{itemize}
    \item still don't provide satisfactory solutions for \daiji{concurrency}
    \item are \daiji{unsafe} (memory-wise, thread-wise, \ldots),
      thus \daiji{not secure}
    \item are rather \daiji{slow}:\\
